...
test_b1_x_pca <- pca_b1_result$x[,1:dims]
```
...
```{r}
#| echo: false
rm(acc, b1_x, b1_y, ctrl, error_rate, pca_b1_result, test_b1_y, test_b1_x, test_b1_x_pca, train_b1_y, train_b1_x, train_b1_x_pca, vst_b1_x, y_hat_lda, dims)
gc();gc()
```

### Testing LDA Model on Centre C1
...
# Tissue types for each sample from centre A1
...
# Apply PCA
pca_c1_result <- prcomp(t(vst_c1_x))

c1_x_pca <- pca_c1_result$x[,1:10]
```
